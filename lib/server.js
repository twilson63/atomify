...
module.exports = function (args) {
  ...
  http.createServer(function (req, res) {
    ...
    // add support to inject proxy handler for /api/* types of data calls
    // server: {
    //   proxy: {
    //     matcher: /^api\//,
    //     handler: function(req, res) {
    //        ... Implement Proxy Here
    //     }
    //   }
    // }
    //
    if (args.server.proxy &&
      args.server.proxy.handler &&
      args.server.proxy.matcher &&
      args.server.proxy.matcher.test(p)) {
      return args.server.proxy.handler(req,res);
    }
